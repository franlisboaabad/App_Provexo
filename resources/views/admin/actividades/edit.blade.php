...
@section('content_header')
    <h1>Actualización de Actividad</h1>
@stop
                    ...
                    <form action="" method="POST" id="form_actividad">
                            ...
                            <select class="form-control select2" style="width: 100%" name="proyecto_id">
                                @foreach ($proyectos as $proyecto)
                                    <option value="{{ $proyecto->id }}"
                                        {{ $actividad->proyecto_id == $proyecto->id ? 'selected' : '' }}>
                        ...
                        <div class="form group pb-3">
                            ...
                            <input type="text" placeholder="Ingrese actividad" name="actividad" id="actividad"
                                class="form-control" required value="{{ old('actividad', $actividad->actividad) }}">
                        </div>

                        <div class="form-group">
                            <label for="">Fecha de facturación</label>
                            <input type="date" name="fecha_facturacion" id="fecha_facturacion"
                                class="form-control" required value="{{ old('fecha_facturacion', $actividad->fecha_facturacion ) }}">
                        </div>

                        <div class="form-group">
                            @csrf
                            <button type="button" id="btn_Edit" class="btn btn-success btn-xs" data-url="{{ route('actividades.update', $actividad ) }}">Editar actividad</button>
